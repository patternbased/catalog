 ...
 */
function SpecialRatePanel({ visible, style }) {
    ...
    const panelClass = useMemo(
        () =>
            classnames('special-rate-panel', {
                'special-rate-panel--visible': visible,
            }),
        [visible]
    );
    ...
    const formDescriptionClass = useMemo(
        () =>
            classnames('special-rate-panel__header__description', {
                'special-rate-panel__header__description--error': !isValid,
            }),
        [isValid]
    );
            ...
            {song && (
                <div className={panelClass} style={style}>
                    {isSuccess ? (
                        <SuccessPanel
                            onClose={() => {
                                dispatch(setState('specialRateOpened', false));
                                setTimeout(() => {
                                    setIsSuccess(false);
                                }, 500);
                            }}
                        ...
                        <div className="special-rate-panel__container">
                            <div className="special-rate-panel__header">
                                <div className="special-rate-panel__header__title">
                                    <CloseIcon
                                        onClick={() => {
                                            dispatch(setState('specialRateOpened', false));
                                        }}
                                    />
                                    Special Rate License
                                </div>
                                <div className="special-rate-panel__header__song">
                                    <img className="special-rate-panel__header__song__image" src={song.image} />
                                    <div className="special-rate-panel__header__song__details">
                                        <div className="special-rate-panel__header__song__details__title">
                                            {song.title}
                                        </div>
                                        <div className="special-rate-panel__header__song__details__artist">
                                            by {song.artist}
                                        </div>
                                    </div>
                                </div>
                                <div className={formDescriptionClass}>
                                    {isValid ? (
                                        'Special rates available for projects regarding the environment, human rights and animal protection. Tell us about your project for a quote.'
                                    ) : (
                                        <>
                            ...
                            <div className="special-rate-panel__form">
                                <Formik
                                    initialValues={{
                                        name: '',
                                        email: '',
                                    ...
                                    onSubmit={(values, { setSubmitting }) => {
                                        const emailData = {
                                            email: values.email,
                                            subject: 'New form submission for PB Special License Rate quote',
...
SpecialRatePanel.displayName = 'SpecialRatePanel';

SpecialRatePanel.propTypes = {
    visible: PropTypes.oneOfType([PropTypes.bool, PropTypes.string]),
    chosenType: PropTypes.string,
    style: PropTypes.object,
};

SpecialRatePanel.defaultProps = {
    visible: false,
    chosenType: '',
    style: {},
};

export default memo(SpecialRatePanel);
